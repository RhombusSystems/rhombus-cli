...
func init() {
	netboxCmd := &cobra.Command{
		Use:   "netbox",
		Short: "Query Lenel S2 NetBox access-control events",
		Long:  "Search Honeywell NetBox badge events (grants and anomalies) recorded as camera seekpoints.",
	}

	eventsCmd := &cobra.Command{
		Use:   "events",
		Short: "Search Lenel S2 NetBox badge events",
		Long: "Search Honeywell NetBox access-control events by time, camera, location, cardholder, badge status, " +
			...
			"Examples:\n" +
			"  rhombus netbox events --after \"24h ago\"\n" +
			"  rhombus netbox events --cardholder \"Lisa Lake\" --after \"7d ago\"\n" +
			"  rhombus netbox events --anomaly-only --camera \"Lobby Cam\"",
		RunE: runNetBoxEvents,
	}
	...
	netboxCmd.AddCommand(eventsCmd)
	rootCmd.AddCommand(netboxCmd)
}

func runNetBoxEvents(cmd *cobra.Command, args []string) error {
	...
	body := map[string]any{
		"limit": maxResults,
		// Scope the unified integration access-event search to NetBox's dedicated activity types.
